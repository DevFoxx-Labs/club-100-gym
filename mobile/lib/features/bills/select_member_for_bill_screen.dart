...
/// Lets the admin pick a member to raise a manual bill for (via the Bills tab FAB).
class SelectMemberForBillScreen extends StatefulWidget {
  const SelectMemberForBillScreen({super.key});

  @override
  State<SelectMemberForBillScreen> createState() => _SelectMemberForBillScreenState();
}

class _SelectMemberForBillScreenState extends State<SelectMemberForBillScreen> {
  ...
  Future<void> _onMemberSelected(MemberModel member) async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => GenerateBillScreen(member: member, membership: membership),
      ),
    );
  }
